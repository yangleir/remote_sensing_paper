...
# Power spectrum
gmt psbasemap -R1/1000/0/15 -JX-2il/2i -Bxa1pf3g3+l"Wavelength:seconds(or m)"  -Byag+l"Power spectrum:m@+2@+/cycle/m(s)" -BWN -K -O -Y2.2i >> $ps3
awk '{print $1,$2}' pow5_pass.txt | gmt psxy -R -JX-2il/2i -K -W2.25p,red -O -i0,1 >> $ps3
awk '{print $1,$2}' pow5_jizai1_x.txt | gmt psxy -R -JX-2il/2i -W2.25p,blue -O -K -i0,1 >> $ps3
gmt psbasemap -R0.001/1/0/15 -JX2il/2i  -BSE -Bxa1pf3+l"Frequency:cycle/seconds(or m)" -O -K>> $ps3


# variance preserving spectrum
...
awk '{print log(1/$1)/2+0.3,2*$2*(1/$1)}' pow5_jizai1_x.txt | gmt psxy -R -J -W2.25p,blue -O -K -i0,1 -t50>> $ps3
gmt psbasemap -R-7/0/-1e-1/4e-1 -JX2i/2i  -BSEwn -Bxaf+l"Frequency:ln(cycle/s)" -O >> $ps3

gmt psconvert $ps3 $ps -A -Tf


rm gmt.* smooth_track* t.d *.ps *.txt *.cpt